...
class BaseRegistry:
    ...
    @property
    def data_converter_type(self) -> Type[DataConverter]:
    ...
    GQL_PAYLOAD_BUILDER = GQLPayloadBuilder()
    ...
    def __init__(self, config):
        ...
        self.client = GrapheneClient(config['user'], self.GQLQuery, self.GQLMutation)
        self.data_converter = self.data_converter_type()
        self.gql_mapper = self.gql_mapper_type(self.fields_mapping, self.special_fields)
    ...
    def get_record(self, mapped_data, fetched_fields=None, only_first=True, after_cursor=None, first=5):
        ...
        query = self.GQL_PAYLOAD_BUILDER.get_single_model_query(self.queries["get"], arguments_with_values, fetched_fields)
    ...
    def manage_registry_record(self, mutation_name, mapped_data_query, mapped_data_write=None):
        ...
        query = self.GQL_PAYLOAD_BUILDER.get_mutation(
